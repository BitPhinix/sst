...
export interface Cloudflare5WorkerArgs {
  /**
  ...
  transform?: {
     ...
     */
    worker?: Transform<cloudflare5.WorkersScriptArgs>;
  };
  /**
   * @internal
 ...
 * ```
 */
export class Cloudflare5Worker extends Component implements Link.Linkable {
  private script: Output<cloudflare5.WorkersScript>;
  private workerUrl: WorkerUrl;
  private workerDomain?: cloudflare5.WorkersCustomDomain;

  constructor(
    name: string,
    args: Cloudflare5WorkerArgs,
    ...
    function createScript() {
      return all([build, args.environment, iamCredentials, bindings]).apply(
        async ([build, environment, iamCredentials, bindings]) => {
          return new cloudflare5.WorkersScript(
            ...transform(
              ...
              `${name}Script`,
              {
                ...
                bindings: [
                  ...(iamCredentials
                    ...
                    : []),
                  ...(bindings.secretTextBindings ?? []).map((binding) => ({
                    type: "secret_text",
                    ...binding,
                  })),
                  ...(bindings.plainTextBindings ?? []).map((binding) => ({
                    type: "plain_text",
                    ...binding,
                  })),
                  ...(bindings.d1DatabaseBindings ?? []).map(
                    ({ databaseId, ...binding }) => ({
                      type: "d1",
                      id: databaseId,
                      ...binding,
                  ...
                  ...(bindings.kvNamespaceBindings ?? []).map((binding) => ({
                    type: "kv_namespace",
                    ...binding,
                  })),
                  ...(bindings.queueBindings ?? []).map((binding) => ({
                    type: "queue",
                    ...binding,
                  })),
                  ...(bindings.r2BucketBindings ?? []).map((binding) => ({
                    type: "r2_bucket",
                    ...binding,
                  })),
                  ...(bindings.serviceBindings ?? []).map((binding) => ({
                    type: "service",
                    ...binding,
                  })),
                  ...(bindings.durableObjectNamespaceBindings ?? []).map(
                    (binding) => ({
    ...
    function createWorkersDomain() {
      ...
      return new cloudflare5.WorkersCustomDomain(
        `${name}Domain`,
        {
...
const __pulumiType = "sst:cloudflare5:Worker";
// @ts-expect-error
Cloudflare5Worker.__pulumiType = __pulumiType;
